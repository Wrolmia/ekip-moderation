...
exports.run = async (client, message, args) => {
  ...
  if(!newState.member.voice || newState.member.voice.channelID != ayarlar.streamer18kanal1) return 
  ...
  if(UyarıVegas === 1) return db.delete(`streameruyarı_${newState.member.id}`) && newState.member.roles.add(ayarlar.streamercezalırolü) && client.channels.cache.get(ayarlar.streamercezalılog).send(`${newState.member} üyesi **18 yaşından** küçük olmasına rağmen +18 kanallara giriş yaptığından dolayı sesten atıldı! Girdiği kanal: ${newState.channel.name}`)
  }
  }catch{{}}
  ...
  if(!newState.member.voice || newState.member.voice.channelID != ayarlar.streamer18kanal2) return 
  if(Number(newState.member.nickname.split('|')[1]) < 18) { 
  newState.member.voice.setChannel(null)
  db.add(`streameruyarı_${newState.member.id}`, 1)
