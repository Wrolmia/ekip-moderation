...
const mutelog = message.guild.channels.cache.find(c => c.id === (ayarlar.vmutelog))
...
if(!kullanici) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, bir kullanıcı belirt @RK/ID gibi.`).setColor('RANDOM').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
