...
exports.run = async(client, message, args) => {
   ...
   let jail = message.guild.roles.cache.find(r => r.id === '835503956263829534')//UNREG ROL GİR

   
   
   let Jaillog = '832978273616134164'
   if(!Jaillog) {
  ...
  while (!uwu) {
   ...
   member.roles.set(["835503956263829534"])
   ...
   .setDescription(`**Jail'den Çıkarılan Kullanıcı : ${member} \nSebep : ${sebep} \nJail'den Çıkaran Yetkili : ${message.author} \nYetkilinin Toplam Jaile Attığı Kişi : \`${jailhyper}\`**`)
   message.channel.send(embed);
   -  
  
    message.guild.channels.cache.get(Jaillog).send(new Discord.MessageEmbed().setAuthor(`KW ❤️`, message.author.avatarURL()).setColor("BLACK").setDescription(`${member} Adlı Üye Jail'den Çıkarıldı\nJail'den Çıkaran Yetkili ${message.author}\nSebep : ${sebep}\nYetkinin Toplam Jaile Attığı Üye Sayısı : \`${jailhyper}\``));
}
